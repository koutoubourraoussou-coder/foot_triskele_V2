...
O25_RANDOM_POOL_BASE_GLOBAL_FILE = Path("data/o25_random_pool_base_global.tsv")
O25_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE = Path("data/o25_random_pool_effective_global.tsv")
TICKETS_O25_RANDOM_TSV_FILE = Path("data/tickets_o25_random.tsv")
TICKETS_O25_REPORT_GLOBAL_FILE = Path("data/tickets_o25_random_report_global.txt")

O15_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE = Path("data/o15_super_random_pool_effective_global.tsv")
TICKETS_O15_SUPER_RANDOM_TSV_FILE = Path("data/tickets_o15_super_random.tsv")
TICKETS_O15_SUPER_RANDOM_REPORT_GLOBAL_FILE = Path("data/tickets_o15_super_random_report_global.txt")

U35_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE = Path("data/u35_super_random_pool_effective_global.tsv")
TICKETS_U35_SUPER_RANDOM_TSV_FILE = Path("data/tickets_u35_super_random.tsv")
TICKETS_U35_SUPER_RANDOM_REPORT_GLOBAL_FILE = Path("data/tickets_u35_super_random_report_global.txt")

O25_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE = Path("data/o25_super_random_pool_effective_global.tsv")
TICKETS_O25_SUPER_RANDOM_TSV_FILE = Path("data/tickets_o25_super_random.tsv")
TICKETS_O25_SUPER_RANDOM_REPORT_GLOBAL_FILE = Path("data/tickets_o25_super_random_report_global.txt")
...
# ✅ Fichiers dispo
RANKINGS_LEAGUE_BET_FILE_CLASSIC = Path("data/rankings/triskele_ranking_league_x_bet.tsv")
RANKINGS_TEAM_BET_FILE_CLASSIC   = Path("data/rankings/triskele_ranking_team_x_bet.tsv")

RANKINGS_LEAGUE_BET_FILE_COMPOSITE = Path("data/rankings/triskele_composite_league_x_bet.tsv")
RANKINGS_TEAM_BET_FILE_COMPOSITE   = Path("data/rankings/triskele_composite_team_x_bet.tsv")
...
class TicketBuildOutput:
    ...
    report_u35: str
    tickets_o25: List[Ticket]
    report_o25: str
    tickets_o15_super: List[Ticket]
    report_o15_super: str
    tickets_u35_super: List[Ticket]
    report_u35_super: str
    tickets_o25_super: List[Ticket]
    report_o25_super: str
    added_sys: int = 0
    added_o15: int = 0
    ...
    added_o25: int = 0
    added_o25_global: int = 0
    added_o15_super: int = 0
    added_o15_super_global: int = 0
    added_u35_super: int = 0
    added_u35_super_global: int = 0
    added_o25_super: int = 0
    added_o25_super_global: int = 0
...
def _extract_rank_rate(row: dict) -> float:
    """
    Priorité :
    - composite_score si présent
    - success_rate sinon
    """
    if row is None:
        return 0.0
    ...
    try:
        return float(row.get("win_rate", 0.0) or 0.0)
    except Exception:
        return 0.0
...
def _load_rankings() -> Tuple[Optional[dict], Optional[dict]]:
    ...
    Accepte :
      - fichiers CLASSIC : success_rate
      - fichiers COMPOSITE : composite_score
    """
    ...
    league_path, team_path = _resolve_ranking_paths()
    ...
    header_classic = {"league", "bet_key", "samples", "success", "fail", "success_rate"}
    header_composite = {"league", "bet_key", "samples", "success", "fail", "base_rate", "goals_score", "composite_score"}

    idx_map, data_start = _detect_header_row(rows, header_classic)
    is_composite = False

    if idx_map is None:
        idx_map, data_start = _detect_header_row(rows, header_composite)
    ...
    for r in rows[data_start:]:
        try:
            ...
            if idx_map:
                ...
                if is_composite:
                    rate = float(cols[idx_map["composite_score"]])
                else:
                    rate = float(cols[idx_map["success_rate"]])
    ...
    header_classic = {"league", "team", "bet_key", "samples", "success", "fail", "success_rate"}
    header_composite = {"league", "team", "bet_key", "samples", "success", "fail", "base_rate", "goals_score", "composite_score"}
...
def _ticket_score_system_league(picks: List[Pick], league_bet: Optional[dict]) -> float:
    if not picks:
        return -1e9
...
def _system_match_mean_team_rate(p: Pick, team_bet: Optional[dict]) -> float:
    ...
    vals: List[float] = []
    for t in teams:
        wr, dec = _team_rate(team_bet, t, league, fam)
        if wr is not None and dec > 0:
            c = _confidence_coef(dec, n_full=5, base=0.70)
            vals.append(float(wr) * c)
...
def _system_priority_weight_team(p: Pick, team_bet: Optional[dict]) -> float:
    mean_wr = _system_match_mean_team_rate(p, team_bet)
...
def _system_ticket_final_score(
    picks: List[Pick],
    league_bet: Optional[dict],
    team_bet: Optional[dict],
) -> float:
...
def _random_ticket_final_score(
    picks: List[Pick],
    league_bet: Optional[dict],
    team_bet: Optional[dict],
) -> float:
    ...
    return _ticket_score_random_team(picks, team_bet)
...
def filter_o25_random_all(picks: List[Pick]) -> List[Pick]:
    out: List[Pick] = []
    for p in picks:
        bk = (p.bet_key or "").strip().upper()
        is_o25 = (bk == O25_CANON) or ("O25" in bk) or ("OVER25" in bk) or ("OVER_2_5" in bk)
        if not is_o25:
            continue
        if p.odd is None:
            continue
...
def filter_effective_super_random_pool(
    picks: List[Pick],
    league_bet: Optional[dict],
) -> List[Pick]:
    """
    Gate LEAGUE UNIQUEMENT pour le super random.
    Pas de gate TEAM ni HYBRID.
    Accepte si : pas de donnees (require_data=False) OU wr >= rnd_min_wr.
    """
    cfg = T()
    rnd_min_wr = (
        cfg.random_league_bet_min_winrate
        if cfg.random_league_bet_min_winrate is not None
        else cfg.league_bet_min_winrate
    )
    ...
    for p in picks:
        ...
        if dec <= 0:
            out.append(p)  # pas de data => passe
            continue

        if wr is not None and float(wr) < float(rnd_min_wr):
            continue  # ligue insuffisante => rejet
...
def _build_super_random_ticket_for_one_day(filtered_picks: List[Pick]) -> Optional[Ticket]:
    ...
    return Ticket(
        picks=selected,
        target_reached=(cumulative >= cfg.target_odd),
        group_no=1,
        option_no=1,
    )
...
def filter_effective_random_pool(
    ...
    for p in picks:
        ...
        if build_src == "TEAM":
            ...
            vals: List[Tuple[Optional[float], int]] = []
            for team in teams:
...
def _random_reject_reason(
    ...
    if build_src == "TEAM":
        teams = _pick_primary_teams(p)
        vals: List[Tuple[str, Optional[float], int]] = []
    ...
    *,
    ...
    for p in pool:
        ...
        if mode_u == "RANDOM":
            ...
            reason = _random_reject_reason(p, league_bet, team_bet)
            if reason:
                res["PERF_REJECT"].append(p)
                res["REASONS"][_reason_key(p)] = reason
                continue
...
def _try_build_ticket_system(
    ...
    *,
    ...
    weights = [_system_generation_weight(p, league_bet, team_bet) for p in pool]
    if not any(w > 0 for w in weights):
        weights = [1.0 for _ in pool]

    def _final_score(ticket_picks: List[Pick]) -> float:
        return _system_ticket_final_score(ticket_picks, league_bet, team_bet)
    ...
    f"=> choisi: {len(chosen)} legs | score={_fmt_score_pct(chosen_score)} | final_source={cfg.system_select_source}"
        ...
        if mlevel >= 3 and chosen:
            ...
            lines: List[str] = []
            lines.append(f"MAESTROLOGUE — Détail du score (ticket choisi, SYSTEM, {select_src} x BET)")
            lines.append("-" * 78)
            lines.append(f"- score ticket: {score:.3f} ({_fmt_pct(score)})")

            if select_src == "LEAGUE":
                lines.append("- formule: wr_adj = wr(league x bet) * coef(decided), coef(1)=0.70 → coef(>=5)=1.00")
            else:
            ...
            for i, p in enumerate(chosen_sorted, start=1):
                ...
                if select_src == "LEAGUE":
                    ...
                    if wr is not None and dec > 0:
                        ...
                        out_lines.append(
                            f"   LEAGUE {lg}: wr={float(wr):.2f} dec={int(dec)} coef={float(coef):.3f} => adj={float(adj):.3f} ({_fmt_pct(adj)})"
                        )
                    else:
                        out_lines.append(
                            f"   LEAGUE {lg}: wr=— dec=0 coef=0.000 => adj=0.000 ({_fmt_pct(0.0)})"
                        )
                else:
                    vals: List[float] = []
                    ...
                    if wr1 is not None and dec1 > 0:
                        ...
                        vals.append(adj1)
                        out_lines.append(
                            f"   HOME {p.home}: wr={float(wr1):.2f} dec={int(dec1)} coef={float(coef1):.3f} => adj={float(adj1):.3f} ({_fmt_pct(adj1)})"
                        )
                    else:
                        out_lines.append(
                            f"   HOME {p.home}: wr=— dec=0 coef=0.000 => adj=0.000 ({_fmt_pct(0.0)})"
                        )
                    ...
                    if wr2 is not None and dec2 > 0:
                        ...
                        vals.append(adj2)
                        out_lines.append(
                            f"   AWAY {p.away}: wr={float(wr2):.2f} dec={int(dec2)} coef={float(coef2):.3f} => adj={float(adj2):.3f} ({_fmt_pct(adj2)})"
                        )
                    else:
                        out_lines.append(
                            f"   AWAY {p.away}: wr=— dec=0 coef=0.000 => adj=0.000 ({_fmt_pct(0.0)})"
                        )
...
def _try_build_ticket_random(
    ...
    *,
    ...
    for p in pool:
        ...
        if build_src == "TEAM":
            ...
            for team in teams:
                ...
                if wr is not None and dec > 0:
                    c = _confidence_coef(int(dec), n_full=5, base=0.70)
                    vals.append(float(wr) * float(c))

            if vals:
                score_by_pick.append(sum(vals) / len(vals))
            else:
                score_by_pick.append(None)

        else:
            wr, dec = _league_bet_rate(league_bet, lg, fam)
            if wr is not None and dec > 0:
                c = _confidence_coef(int(dec), n_full=5, base=0.70)
                score_by_pick.append(float(wr) * float(c))
            else:
                score_by_pick.append(None)

    valid = [w for w in score_by_pick if w is not None]
    mn = min(valid) if valid else None
    mx = max(valid) if valid else None

    def _weight_from_rank_signal(rank_signal: float | None, odd: Optional[float]) -> float:
        if rank_signal is None or mn is None or mx is None or mx == mn:
            w_perf = 1.0
        else:
            w_perf = 1.0 + (float(rank_signal) - float(mn)) / (float(mx) - float(mn))
    ...
    weights = [_weight_from_rank_signal(w, p.odd) for w, p in zip(score_by_pick, pool)]
    if not any(w > 0 for w in weights):
        weights = [1.0 for _ in pool]

    rejected_by_build_gate = 0

    def _build_exact(order: List[Pick], wanted_legs: int) -> Optional[List[Pick]]:
        nonlocal rejected_by_build_gate
    ...
    def _final_score(ticket_picks: List[Pick]) -> float:
        return _random_ticket_final_score(ticket_picks, league_bet, team_bet)
    ...
    if mlevel >= 2:
        ...
        lines.append("-" * 64)
        lines.append(
            f"- build_source: {build_src} x BET >= {cfg.league_bet_min_winrate:.2f} (require_data={cfg.league_bet_require_data})"
        )
        lines.append(
            f"- select_source: {select_src} x BET (wr_adj = wr * coef(decided), coef(1)=0.70 → coef(>=5)=1.00)"
        )
        lines.append(f"- règle: préférer 3L si écart <= {cfg.prefer_3legs_delta*100:.2f}%")
        lines.append(f"- iterations: {it} | valid_3L_found: {found3} | valid_4L_found: {found4}")
        lines.append(f"- top3_size: {len(items3)} | top4_size: {len(items4)}")
        lines.append(f"- rejected_by_build_gate (approx): {rejected_by_build_gate}")
        lines.append("")
        ...
        if chosen:
            lines.append(
                f"=> choisi: {len(chosen)} legs | score={_fmt_score_pct(chosen_score)} | final_source={select_src}"
            )
            lines.append("")
            lines.append("Ticket choisi (aperçu)")
            lines.append("-" * 22)
    ...
    if mlevel >= 3 and chosen:
        chosen_sorted = sorted(chosen, key=lambda p: _time_to_minutes(p.time_str))
        score = _random_ticket_final_score(chosen_sorted, league_bet, team_bet)

        lines: List[str] = []
        lines.append(f"MAESTROLOGUE — RANDOM — Détail du score (ticket choisi, {select_src} x BET)")
        lines.append("-" * 78)
        lines.append(f"- score ticket: {score:.3f} ({_fmt_pct(score)})")
        lines.append("- formule: wr_adj = wr * coef(decided), coef(1)=0.70 → coef(>=5)=1.00")
        lines.append("")
        ...
        for i, p in enumerate(chosen_sorted, start=1):
            ...
            if select_src == "LEAGUE":
                ...
                if wr is not None and dec > 0:
                    ...
                    out_lines.append(f"   LEAGUE {lg}: wr=— dec=0 coef=0.000 => adj=0.000 ({_fmt_pct(0.0)})")
            else:
                vals: List[float] = []
                ...
                if wr1 is not None and dec1 > 0:
                    ...
                    out_lines.append(f"   HOME {p.home}: wr=— dec=0 coef=0.000 => adj=0.000 ({_fmt_pct(0.0)})")
                ...
                if wr2 is not None and dec2 > 0:
                    ...
                    out_lines.append(f"   AWAY {p.away}: wr=— dec=0 coef=0.000 => adj=0.000 ({_fmt_pct(0.0)})")
...
def generate_tickets_from_tsv(
    ...
    try:
        ...
        tickets_o25_report_path = _run_scoped_or_data("tickets_o25_random_report.txt")
        ...
        if not fast_mode:
            added_o15_super_pool = append_playable_picks_to_global(
                picks=o15_super_pool_effective,
                global_path=O15_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE,
                pipeline_name="O15_SUPER_RANDOM_POOL_EFFECTIVE",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_o15_super_pool:
                print(f"📦 [O15_SUPER_RANDOM] Pool effectif : {O15_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE} (+{added_o15_super_pool})")
            else:
                print(f"ℹ️ [O15_SUPER_RANDOM] Pool effectif inchange : {O15_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE}")

        tickets_o15_super = build_super_random_tickets(o15_super_pool_effective)

        if not fast_mode:
            added_o15_super = write_tickets_tsv(tickets_o15_super, TICKETS_O15_SUPER_RANDOM_TSV_FILE, id_suffix="O15SR")
            if added_o15_super:
                print(f"✅ [O15_SUPER_RANDOM] Tickets TSV : {TICKETS_O15_SUPER_RANDOM_TSV_FILE} (+{added_o15_super})")
            else:
                print("⚠️ [O15_SUPER_RANDOM] Aucun ticket TSV (tickets=0 ou dedup).")

            report_o15_super = render_tickets_report(
                tickets_o15_super,
                title=f"TICKETS TRISKELE{suffix} — O15_SUPER_RANDOM — depuis {Path(tsv_path).name}",
                id_suffix="O15SR",
            )
            tickets_o15_super_report_path = _run_scoped_or_data("tickets_o15_super_random_report.txt")
            _write_report_robust(
                run_path=tickets_o15_super_report_path,
                data_path=Path("data/tickets_o15_super_random_report.txt"),
                text=report_o15_super,
            )
            print(f"📝 [O15_SUPER_RANDOM] Report ecrit : {tickets_o15_super_report_path}")

            added_o15_super_global = append_report_to_global(
                report_text=report_o15_super,
                global_path=TICKETS_O15_SUPER_RANDOM_REPORT_GLOBAL_FILE,
                pipeline_name="O15_SUPER_RANDOM",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_o15_super_global:
                print(f"🧾 [O15_SUPER_RANDOM] Global alimente : {TICKETS_O15_SUPER_RANDOM_REPORT_GLOBAL_FILE} (+{added_o15_super_global})")
            else:
                print(f"ℹ️ [O15_SUPER_RANDOM] Global inchange : {TICKETS_O15_SUPER_RANDOM_REPORT_GLOBAL_FILE}")
        else:
            added_o15_super = 0
            added_o15_super_global = 0
            report_o15_super = ""
        ...
        if not fast_mode:
            added_u35_super_pool = append_playable_picks_to_global(
                picks=u35_super_pool_effective,
                global_path=U35_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE,
                pipeline_name="U35_SUPER_RANDOM_POOL_EFFECTIVE",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_u35_super_pool:
                print(f"📦 [U35_SUPER_RANDOM] Pool effectif : {U35_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE} (+{added_u35_super_pool})")
            else:
                print(f"ℹ️ [U35_SUPER_RANDOM] Pool effectif inchange : {U35_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE}")

        tickets_u35_super = build_super_random_tickets(u35_super_pool_effective)

        if not fast_mode:
            added_u35_super = write_tickets_tsv(tickets_u35_super, TICKETS_U35_SUPER_RANDOM_TSV_FILE, id_suffix="U35SR")
            if added_u35_super:
                print(f"✅ [U35_SUPER_RANDOM] Tickets TSV : {TICKETS_U35_SUPER_RANDOM_TSV_FILE} (+{added_u35_super})")
            else:
                print("⚠️ [U35_SUPER_RANDOM] Aucun ticket TSV (tickets=0 ou dedup).")

            report_u35_super = render_tickets_report(
                tickets_u35_super,
                title=f"TICKETS TRISKELE{suffix} — U35_SUPER_RANDOM — depuis {Path(tsv_path).name}",
                id_suffix="U35SR",
            )
            tickets_u35_super_report_path = _run_scoped_or_data("tickets_u35_super_random_report.txt")
            _write_report_robust(
                run_path=tickets_u35_super_report_path,
                data_path=Path("data/tickets_u35_super_random_report.txt"),
                text=report_u35_super,
            )
            print(f"📝 [U35_SUPER_RANDOM] Report ecrit : {tickets_u35_super_report_path}")

            added_u35_super_global = append_report_to_global(
                report_text=report_u35_super,
                global_path=TICKETS_U35_SUPER_RANDOM_REPORT_GLOBAL_FILE,
                pipeline_name="U35_SUPER_RANDOM",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_u35_super_global:
                print(f"🧾 [U35_SUPER_RANDOM] Global alimente : {TICKETS_U35_SUPER_RANDOM_REPORT_GLOBAL_FILE} (+{added_u35_super_global})")
            else:
                print(f"ℹ️ [U35_SUPER_RANDOM] Global inchange : {TICKETS_U35_SUPER_RANDOM_REPORT_GLOBAL_FILE}")
        else:
            added_u35_super = 0
            added_u35_super_global = 0
            report_u35_super = ""
        ...
        if not fast_mode:
            added_o25_random_pool_base = append_playable_picks_to_global(
                picks=o25_random_pool_base,
                global_path=O25_RANDOM_POOL_BASE_GLOBAL_FILE,
                pipeline_name="O25_RANDOM_POOL_BASE",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_o25_random_pool_base:
                print(f"📦 [O25_RANDOM_ALL] Pool base exporté : {O25_RANDOM_POOL_BASE_GLOBAL_FILE} (+{added_o25_random_pool_base} lignes)")
            else:
                print(f"ℹ️ [O25_RANDOM_ALL] Pool base inchangé : {O25_RANDOM_POOL_BASE_GLOBAL_FILE}")

            added_o25_random_pool_effective = append_playable_picks_to_global(
                picks=o25_random_pool_effective,
                global_path=O25_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE,
                pipeline_name="O25_RANDOM_POOL_EFFECTIVE",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_o25_random_pool_effective:
                print(f"📦 [O25_RANDOM_ALL] Pool effectif exporté : {O25_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE} (+{added_o25_random_pool_effective} lignes)")
            else:
                print(f"ℹ️ [O25_RANDOM_ALL] Pool effectif inchangé : {O25_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE}")
        ...
        if not fast_mode:
            ...
            if added_o25:
                ...
                print("⚠️ [O25_RANDOM_ALL] Aucun ticket TSV écrit (tickets=0 ou dédup).")

            report_o25 = render_tickets_report(
                tickets_o25,
                title=f"TICKETS TRISKÈLE{suffix} — O25_RANDOM_ALL — depuis {Path(tsv_path).name}",
                id_suffix="O25R",
            )
            _write_report_robust(
                run_path=tickets_o25_report_path,
                data_path=Path("data/tickets_o25_random_report.txt"),
                text=report_o25,
            )
            print(f"📝 [O25_RANDOM_ALL] TicketsReport écrit : {tickets_o25_report_path} + data/tickets_o25_random_report.txt")

            added_o25_global = append_report_to_global(
                report_text=report_o25,
                global_path=TICKETS_O25_REPORT_GLOBAL_FILE,
                pipeline_name="O25_RANDOM_ALL",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_o25_global:
                print(f"🧾 [O25_RANDOM_ALL] Global report alimenté : {TICKETS_O25_REPORT_GLOBAL_FILE} (+{added_o25_global} tickets)")
            else:
                print(f"ℹ️ [O25_RANDOM_ALL] Global report inchangé (0 nouveau ticket) : {TICKETS_O25_REPORT_GLOBAL_FILE}")
        else:
            added_o25 = 0
            added_o25_global = 0
            report_o25 = ""
        ...
        if not fast_mode:
            added_o25_super_pool = append_playable_picks_to_global(
                picks=o25_super_pool_effective,
                global_path=O25_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE,
                pipeline_name="O25_SUPER_RANDOM_POOL_EFFECTIVE",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_o25_super_pool:
                print(f"📦 [O25_SUPER_RANDOM] Pool effectif : {O25_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE} (+{added_o25_super_pool})")
            else:
                print(f"ℹ️ [O25_SUPER_RANDOM] Pool effectif inchange : {O25_SUPER_RANDOM_POOL_EFFECTIVE_GLOBAL_FILE}")

        tickets_o25_super = build_super_random_tickets(o25_super_pool_effective)

        if not fast_mode:
            added_o25_super = write_tickets_tsv(tickets_o25_super, TICKETS_O25_SUPER_RANDOM_TSV_FILE, id_suffix="O25SR")
            if added_o25_super:
                print(f"✅ [O25_SUPER_RANDOM] Tickets TSV : {TICKETS_O25_SUPER_RANDOM_TSV_FILE} (+{added_o25_super})")
            else:
                print("⚠️ [O25_SUPER_RANDOM] Aucun ticket TSV (tickets=0 ou dedup).")

            report_o25_super = render_tickets_report(
                tickets_o25_super,
                title=f"TICKETS TRISKELE{suffix} — O25_SUPER_RANDOM — depuis {Path(tsv_path).name}",
                id_suffix="O25SR",
            )
            tickets_o25_super_report_path = _run_scoped_or_data("tickets_o25_super_random_report.txt")
            _write_report_robust(
                run_path=tickets_o25_super_report_path,
                data_path=Path("data/tickets_o25_super_random_report.txt"),
                text=report_o25_super,
            )
            print(f"📝 [O25_SUPER_RANDOM] Report ecrit : {tickets_o25_super_report_path}")

            added_o25_super_global = append_report_to_global(
                report_text=report_o25_super,
                global_path=TICKETS_O25_SUPER_RANDOM_REPORT_GLOBAL_FILE,
                pipeline_name="O25_SUPER_RANDOM",
                run_date=run_date,
                source_tsv=tsv_path,
            )
            if added_o25_super_global:
                print(f"🧾 [O25_SUPER_RANDOM] Global alimente : {TICKETS_O25_SUPER_RANDOM_REPORT_GLOBAL_FILE} (+{added_o25_super_global})")
            else:
                print(f"ℹ️ [O25_SUPER_RANDOM] Global inchange : {TICKETS_O25_SUPER_RANDOM_REPORT_GLOBAL_FILE}")
        else:
            added_o25_super = 0
            added_o25_super_global = 0
            report_o25_super = ""

        if _maestro_level() >= 1:
            foot = (
                f"RUN SUMMARY\n"
                f"- system: {len(tickets_system)} | o15: {len(tickets_o15)} | o15_super: {len(tickets_o15_super)}"
                f" | u35: {len(tickets_u35)} | u35_super: {len(tickets_u35_super)}"
                f" | o25: {len(tickets_o25)} | o25_super: {len(tickets_o25_super)}\n"
                f"{'-'*40}\n\n"
            )
        ...
        return TicketBuildOutput(
            ...
            tickets_o25=tickets_o25,
            report_o25=report_o25,
            tickets_o15_super=tickets_o15_super,
            report_o15_super=report_o15_super,
            tickets_u35_super=tickets_u35_super,
            report_u35_super=report_u35_super,
            tickets_o25_super=tickets_o25_super,
            report_o25_super=report_o25_super,
            added_sys=added_sys,
            added_o15=added_o15,
            ...
            added_o25=added_o25,
            added_o25_global=added_o25_global,
            added_o15_super=added_o15_super,
            added_o15_super_global=added_o15_super_global,
            added_u35_super=added_u35_super,
            added_u35_super_global=added_u35_super_global,
            added_o25_super=added_o25_super,
            added_o25_super_global=added_o25_super_global,
        )
