"""
BCEA Session 15 — Seuil min équipe pool RANDOM
================================================
Teste l'impact de baisser le seuil d'inclusion équipe dans le pool RANDOM.

Contexte :
  - Profil champion : RANDOM_BUILD_SOURCE=TEAM, league_bet_min_winrate=0.60
  - Filtre actuel : min(wr_eq1, wr_eq2) >= 0.60  → si une équipe < 60%, rejet
  - Aujourd'hui : 27/91 picks rejetés (30%) à cause de ce filtre

Variantes testées :
  - Baseline : league_bet_min_winrate = 0.60 (profil champion)
  - Test A    : league_bet_min_winrate = 0.50 (plus permissif)
  - Test B    : league_bet_min_winrate = 0.40 (très permissif)

Usage :
    python -u bcea_session15_random_threshold.py --runs 50 --jobs 6
"""
...
ALL_PROFILES_PATH = ROOT / "data" / "optimizer" / "optimizer_top_profiles.json"
BANKROLL0  = 100.0
MAX_LOSSES = 4
...
def _load_ameliore1() -> BuilderTuning:
    ...
    # Améliorations confirmées du profil champion
    return replace(base,
        two_team_high          = 0.90,
...
def _build_variants(base: BuilderTuning):
    """
    BCEA Session 15 — seuil min équipe pool RANDOM
    Le paramètre league_bet_min_winrate est utilisé dans filter_effective_random_pool
    (TEAM mode) comme seuil : min(wr_eq1, wr_eq2) >= league_bet_min_winrate
    """
    return [
        ("Baseline — seuil=0.60 (champion)",  base),
        ("Test A   — seuil=0.50",              replace(base, league_bet_min_winrate=0.50)),
        ("Test B   — seuil=0.40",              replace(base, league_bet_min_winrate=0.40)),
    ]


# ─── Martingale ───────────────────────────────────────────────────────────────

def _simulate_martingale_normal(sequence):
    ...
    for is_win, odd in sequence:
        ...
        prev_stake = stake
    return {"multiple": round(bankroll/BANKROLL0, 3),
            "n_wins": n_wins, "n_losses": n_losses,
            "max_loss_streak": max_ls, "max_win_streak": max_ws,
            "ruined": bankroll <= 0}


def _simulate_martingale_safe(sequence):
    ...
    for is_win, odd in sequence:
        ...
        prev_stake = stake
    total = ba + reserves
    return {"multiple": round(total/BANKROLL0, 3),
            "n_wins": n_wins, "n_losses": n_losses,
            "max_loss_streak": max_ls, "max_win_streak": max_ws,
...
def main():
    ...
    print(f"[BCEA S15] Seuil min équipe pool RANDOM — {len(variants)} variantes")
    for name, _ in variants:
        print(f"  · {name}")

    datasets = discover_datasets(args.archive_dir, max_days=None)
    print(f"[BCEA S15] {len(datasets)} jours  |  {args.runs} runs/variante  |  {args.jobs} jobs\n")
    ...
    if args.jobs == 1:
        ...
        for vi, (vname, tuning) in enumerate(variants):
            for _ in range(args.runs):
                ...
                print(f"[BCEA S15] {done}/{total_jobs} | {time.time()-start:.0f}s  {vname[:40]}", flush=True)
    else:
        future_map = {}
        with ProcessPoolExecutor(max_workers=args.jobs) as ex:
            ...
            for fut in as_completed(future_map):
                ...
                if done % step == 0 or done == total_jobs:
                    ...
                    print(f"[BCEA S15] {done}/{total_jobs} | {elapsed/60:.1f}min | ETA ~{eta/60:.1f}min", flush=True)

    print(f"\n[BCEA S15] Terminé en {(time.time()-start)/60:.1f}min\n")

    lines = [
        "=" * 72,
        "BCEA Session 15 — Seuil min équipe pool RANDOM",
        f"Date : {date.today()}  |  {args.runs} runs/variante  |  {len(datasets)} jours",
        "Paramètre : league_bet_min_winrate  (min des deux équipes dans RANDOM TEAM mode)",
        "Base : Amélioré #1 champion (0.60)",
        "=" * 72, "",
    ]
    ...
    out_path = OUTPUT_DIR / f"bcea_session15_random_threshold.txt"
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_path.write_text(output, encoding="utf-8")
    print(f"\n[BCEA S15] Résultats → {out_path}")
